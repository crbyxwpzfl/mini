# set up alexa app on tv so tv stays in light sleep not deep sleep so api is responsive at all times even after long times of inactivety
...
def set():  # when wall off switch hdmi also turns wall    when wall on presspowerbutton or set vloume
    ...
    if d['Brightness'] >= 1 and sys.argv[3] == 'Brightness' and sys.argv[4] != '0':  #  set volume when wall on and dont change volume to 0 when shutting off
        requests.post(f'https://{secs.wallip}:1926/6/audio/volume', timeout=2, json={'muted': 'false', 'current': int(sys.argv[4]) }, verify=False, auth=HTTPDigestAuth(secs.walluser, secs.wallpw)); sys.exit()
