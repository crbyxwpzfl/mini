...
def Set():
    ...
    response = requests.post(f'http://{secs.wallip}:1925/6/ambilight/cached', timeout=2, json={'r': int(r*255),'g': int(g*255),'b': int(b*255)})

def reference():
    response = requests.get(f'https://{secs.wallip}:1926/6/ambilight/cached', timeout=2, verify=False, auth=HTTPDigestAuth(secs.walluser, secs.wallpw))
