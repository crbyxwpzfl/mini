...
def pluses(): # TODO debug
    ...
    if d.get('vpnto', "what u want wh")[-2:] != d.get('Current server', "where u are")[:2]: 
        sub(d['sshpi']  + "nordvpn " + d.get('vpnto', "disconnect"), True)
    ...
    if d.get('vpnto', "what u want wh")[-2:] == d.get('Current server', "where u are")[:2] and d['Status'] == "Connected" and d['dlpurls'] and len(sub("killall -s Python", True).split('kill')) == 2:  # +1 account for list.split always len 1 and +1 for Python currently running so this means if no dlp is up
        # TODO remove run(f"osascript -e 'tell app \"Terminal\" to do script \"{pathlib.Path(__file__).resolve()} dlp\" ' ") # call itself and bring dlp() up in new window
...
def vpnstate(): # pipe vpn status into dict
    # TODO remove nicelist = run(d['sshpi'] + "nordvpn status").lstrip('\r-\r  \r\r-\r  \r').rstrip('\n').split('\n') # get vpn status and clean up output a bit
...
def pushsite(): # pull all repos and push changes of overwritesite()
    # TODO remove run(d['gitcssh'] + f" -C {os.path.join(d['puthere'], 'reposetories', 'spinala')} pull")
    ...
    # TODO remove run(f"git -C {os.path.join(d['puthere'], 'reposetories', 'spinala')} commit -am \"site update\" ; " + d['gitcssh'] + f" -C {os.path.join(d['puthere'], 'reposetories', 'spinala')} push ;" )
    ...
    # TODO remove run(f"osascript -e 'tell application \"Messages\" to send \"{d.get('vpnto', 'off')} https://crbyxwpzfl.github.io/spinala/\" to participant \"{d['phonenr']}\"'")
...
def aria(): # TODO perhaps use more advanced opts add trackers and optimize concurrent downloads and save savefile every sec or so
    ...
    for stopped in json.loads(d['r'].content)['result'][1][0]: # man im numb all this nested list dict shit braeks me here we want the first list in the second list in r content result list
        ...
        for fs in stopped.get('files', [{'path':'nofile'}]):
            ...
            if not d['ariaurls']: sub(f"osascript -e 'display notification \"done {d['message']}\" with title \"aria\"'", False) # dont wait on completion just fire notification # only on aria completion call so when no parsing happend so ther is no d['ariaurls']
...
def head():
    parsereadlist() # waht u want vpn location and urls
    vpnstate() # where u are                                                                TODO remove run(d['sshpi']  + "nordvpn " + d.get('vpnto', "disconnect"))
    ...
    if d.get('vpnto', "what u want wh")[-2:] == d.get('Current server', "where u are")[:2] and d['Status'] == "Connected" and d['ariaurls']: # dont do aria() when parsereadlist() vpn state not vpnstate() eg off not stil on cause vpnstate() is lagging actual vpn is off and bunch others
        aria()
                                                                                                                                                # TODO remove run("killall -s Python").split('kill')
    if d.get('vpnto', "what u want wh")[-2:] == d.get('Current server', "where u are")[:2] and d['Status'] == "Connected" and d['dlpurls'] and len(sub("killall -s Python", True).split('kill')) == 2:  # +1 account for list.split always len 1 and +1 for Python currently running so this means if no dlp is up
        # TODO remove run(f"osascript -e 'tell app \"Terminal\" to do script \"{pathlib.Path(__file__).resolve()} dlp\" ' ") # call itself and bring dlp() up in new window
        sub(f"osascript -e 'tell app \"Terminal\" to do script \"{pathlib.Path(__file__).resolve()} dlp\" ' ", False) # dont wait until completion call itself and bring dlp() up in new window
