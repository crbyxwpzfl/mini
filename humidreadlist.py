...
def tapback(message, emote):  # this is inline just for simplyfinging edits for futur ui changes (like/2/2001 dislike/3/2002 !!/5/2004 ?/6/2005)  # NOTE searches for first 100 chars of message
    ...
    if "execution error" in d['title']: sub("""osascript -e 'quit app "Messages"' -e 'delay 2' -e 'tell application "Messages" to activate'""", True); sys.exit()  # tapback error makes messages restart and exits so nothing happes
...
def head():  # TODO perhaps to much tapbacks and need to sys.exit early # runs all for loops once so worst case cleanups.tapback(!!) + cleanups.tapback(delete) + todos.tapback(dislike) + waitings.tapback(like)
    parsereadlist(); d['screens'](); d['pingout'] = d['locaway']()   # since u cannot assign inside lambda but like to not ping multible times
    ...
    for cleanups in [m for m in d['sqllist'] if 2004 in m]:
        if cleanups[0].startswith('http'):                tapback(celanups[0], 5); tapback(cleanups[0], False); sub(f'screen -X -S {cleanups[1]} quit', True);      break  # http message and has !! (from me) - specific screen off
        if cleanups[0].startswith('to') and d['pingout']: tapback(celanups[0], 5); tapback(cleanups[0], False); sub(d['sshspinala'](todos[1], 'disconnect'), True); break  # to message and has !! (from me) and vpn currently on - vpn off
    ...
    for todos in [m for m in d['sqllist'] if None in m]:  #TODO before if None in m istead of if not m[3] # vpn should be on top cause of sql sort
        if todos[0].startswith('http') and d['pingout'] and d['screens']().count('(Detached)') < 6: tapback(todos[0], 3); sub(f'mkdir {d["outdir"](todos[1])} && cd {d["outdir"](todos[1])} && screen -L -S {todos[1]} -d -m dl {todos[0]}', True); break  # message starts 'http' and has None tapback and vpn currently on and screen sockets less than 6 - dl on
    ...
    for waitings in [m for m in d['sqllist'] if 2002 in m and m[1] not in d['screens']]:  # has dislike and has no active screen
        tapback(waitings[0], 2) if waitings[0].startswith('http') and d['searchfiles'](d['files'](  [f.path for f in os.scandir(d['outdir']) if f.path.endswith(waitings[1])][1], 'mp4', 'mp4'  ), 'mp4') else tapback(waitings[0], 6); break  # message 'http....' has no screen and in /outdir/subdir with end of m.date/ has mp4s -> tapback like else tapback ?
...
d = {'get': head, 'dl': dl, 'sort': sort, # defs for running directly in cli via arguments
    ...
    'outdir':       lambda date:                os.path.join('/Users/mini/Downloads/temps/', f'{d.get("title","")[:20].replace("."," ")}{" "+date if d.get("title") else ""}'),  # form http://final.file.whatever/url either makes /outdir/final file whatever date/ or just gives base /outdir to waitings to search /outdir/scan for subdir with date/ for existance of mp4s
